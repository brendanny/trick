...
Policy v15 supports `bool`, `char`, `signed char`, `unsigned char`, `short`,
...
qualifiers and reference fields remain unsupported coverage. Single builtin/enum
...
Single builtin/enum pointers and their fixed arrays are supported as described below.
...
Policy v15 supports the separate `outputs: ["template-attributes"]` profile. Supply
...
## Single enum pointer storage

Policy v15 adds `enum_pointer_storage`, combining a canonical `pointer_type_id`,
builtin-independent `element_type_id` and required `enum_id`. The pointee passes
the same named/nonempty 32-bit enum storage checks as ordinary enum values.
Only the explicitly selected, included enum declaration can supply its table;
ignored or unselected dependencies fail with `ICG_POLICY_ENUM_DEPENDENCY`.
I/O-disabled fields are still omitted before resolving storage dependencies.

Fixed dimensions remain positive and exclude the final legacy zero pointer index,
with at most seven outer dimensions. The C++ type retains its pointer, for example
`ns::Mode*[2][3]`, while `type_name` remains `ns::Mode`. Closed schema validation
and exact replay prevent a rehashed model from changing pointer identity, enum
identity, shape or native type. Prior policy documents must be resolved again.
