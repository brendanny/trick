#!/usr/bin/env python3
"""Compare audited builtin pointer metadata against legacy and native storage."""
...
def pointer(name, kind, offset, dimensions=(), **kwargs):
    return dict(
        arrays.field(name, kind, offset, (*dimensions, 0), **kwargs), pointer=True
    )
...
def mutations(candidate):
    return {
        ...
        "pointer-extent": candidate.replace(
            "0, NULL, 1, {{0, 0}", "0, NULL, 1, {{1, 0}", 1
        ),
        "pointer-rank": candidate.replace(
            "0, NULL, 1, {{0, 0}", "0, NULL, 0, {{0, 0}", 1
        ),
        "pointer-offset": candidate.replace("48, NULL, 1,", "56, NULL, 1,", 1),
        ...
        "pointer-units": candidate.replace(
            '"PointerModel_position", "m"', '"PointerModel_position", "rad"', 1
        ),
    }
...
def capture(extractor: Path, output: Path, compiler: Path) -> dict:
    ...
    case, _ = arrays.reference(HERE, "pointers")
    rejected = {}
    for label, changed in mutations(candidate).items():
        ...
        try:
            ...
            if commands[-1]["returncode"] == 0 and "PointerModel::" not in str(error):
                raise b.BaselineError(
