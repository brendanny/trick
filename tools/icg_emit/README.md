...
This standalone development backend consumes **facts v12, resolved policy v2,
...
in the [policy documentation](../icg_policy/README.md). Old policy v1 documents
...
- Compile-time ABI, record/enum size/alignment, and enum-value checks. Public
...
The emitter supports standard-layout records in the bounded scalar policy and
unscoped enum values representable by `ENUM_ATTR.int`. Its target profile is
...
Scoped enum labels, non-standard-layout records, wider enum values, unsafe packed
...
modifiers, omitted fields/entry points, incorrect C++ linkage, and sentinels must
...
General annotations, scoped enums, inheritance, template/STL emission, and
