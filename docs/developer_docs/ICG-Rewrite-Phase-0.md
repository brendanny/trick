...
| Rewrite-generated legacy metadata | Bounded emitter implemented and compiled independently against legacy/native observations. Policy v3 adds explicit scoped-enum decisions and an [enum corpus](../../tools/icg_baseline/enums/README.md) with six separate snapshots, ten compatible tables / 19 enumerators, opaque/private exclusions and explicit unsigned-narrow/boolean numeric rejections. Policy v4 / emitter v3 additionally compare fixed arrays and aliases in two records / nine fields, including dimension order and corrected namespace-independent UnitsMap keys; see the [array corpus](../../tools/icg_baseline/arrays/README.md). Broader records/annotations, lifecycle, STL and production integration remain pending; see the [milestone](ICG_REWRITE_PLAN.md#201-next-milestone-generate-and-execute-legacy-metadata) |
