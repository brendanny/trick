...
| Legacy metadata versus extracted facts | Three fingerprinted headers: six record tables/sizes, six fields, two enum tables, and explicit record/enum exclusions. Actual captured C++ compiles against real Trick headers/UnitsMap; initialization/size entry points, compiled metadata, and independent native layouts/enum constants agree with facts. Compiler/dependency evidence and negative mutations are retained. Annotation policy, lifecycle execution, template/STL coverage, and broader generated behavior remain pending |
...
| LLVM 17–23 extractor adapters | Localized API adapters; every major in Linux/macOS CI, component linkage at 17/22/23, exact same-platform comparison of eight validated fixture graphs, and package/version artifacts. LLVM 17 remains the floor; this does not establish every host/compiler/package combination |
| GCC 8.5/12 layout and generated-operation probes | Focused native size/alignment/public base-path, special-member type-trait, concrete-template layout, and compiled legacy metadata/init/size probes wired into host CI; lifecycle execution and broader generated-operation gates pending |
