...
unsigned-int bitfields, fixed arrays/aliases, and single builtin/32-bit-enum pointers in ordinary records. `wchar_t`, `char32_t`,
...
The `wchar_t` rejection is a deliberate compatibility divergence from successful
legacy emission, recorded in [ICG-003](architecture/ICG-003-wide-character-compatibility.md).
Affected simulations require migration before production adoption; this is tracked
separately from uncharacterized storage. Legacy omission explains the distinct
`char32_t` rejection.
...
| Rewrite-generated legacy metadata | Compiled legacy/candidate/native comparisons cover the 15 scalar types above, fixed arrays and aliases, unsigned bitfields, and enum tables. The [enum corpus](../../tools/icg_baseline/enums/README.md) includes ten compatible tables / 19 enumerators and explicit numeric rejections; the [array corpus](../../tools/icg_baseline/arrays/README.md) includes two records / nine fields. Policy v17 / emitter v16 also generate bounded template dependencies with first-use symbols and guarded real MemoryManager initialization; five tables replace legacy definitions in the configured checkpoint comparison. Single builtin pointers have 18 audited rows and native/checkpoint gates, including legacy character-pointer string behavior. The enum pointer corpus adds 12 rows, three enum tables and exact address/value restoration; the record pointer corpus adds twelve rows with guarded self/mutual dependencies and cyclic native-address restoration. Two-level builtin pointers add 19 rows and exact slot/terminal checkpoint checks, including legacy terminal-string behavior. References remain rejected. Opt-in lifecycle exports have independent native and configured runtime gates. `wchar_t` requires migration under ICG-003; `char32_t` retains its explicit legacy-omission rejection. Broader records/annotations, STL and production integration remain pending; see the [milestone](ICG_REWRITE_PLAN.md#201-next-milestone-generate-and-execute-legacy-metadata) |
