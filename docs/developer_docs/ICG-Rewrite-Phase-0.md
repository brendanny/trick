...
| Legacy metadata versus extracted facts | Three fingerprinted headers: six record tables/sizes, six fields, two enum tables, and explicit record/enum exclusions. Actual captured C++ compiles against real Trick headers/UnitsMap; initialization/size entry points, compiled metadata, and independent native layouts/enum constants agree with facts. Compiler/dependency evidence and negative mutations are retained. Bounded resolved selection and units/I/O now agree with these observations; general annotations, lifecycle emission, template/STL coverage and broader generated behavior remain pending |
| Policy input evidence | Explicit file requests and source-located roots, exact preprocessor comment spans, observed legacy policy environment, and structured concrete friend evidence implemented in facts v12. Real embedded/template headers and a multi-header synthetic input are exercised. Bounded resolved policy v1 characterizes eligibility, comment association, exclusions, units/I/O and operation-specific friend access with 33 live legacy cases. General policy remains pending |
| Rewrite-generated legacy metadata | Pending; current compiled differential and configured simulation evidence executes legacy output. The [next milestone](ICG_REWRITE_PLAN.md#201-next-milestone-generate-and-execute-legacy-metadata) now has explicit bounded resolved policy; a narrow emitter tested against those observations remains next |
