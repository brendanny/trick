...
| Policy input evidence | Explicit file requests and source-located roots, exact preprocessor comment spans, observed legacy policy environment, and structured concrete friend evidence implemented in facts v12. Real embedded/template headers and a multi-header synthetic input are exercised. Legacy eligibility, comment association/precedence, and operation access policy remain pending |
| Rewrite-generated legacy metadata | Pending; current compiled differential and configured simulation evidence executes legacy output. The [next milestone](ICG_REWRITE_PLAN.md#201-next-milestone-generate-and-execute-legacy-metadata) uses source/selection/friend evidence to add explicit resolved policy and a narrow emitter tested against those observations |
